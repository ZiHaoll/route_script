...
{
	#c获取当前的网关配置
	local current_dhcp_dns1=`nvram get dhcp_dns1_x` 
	if [ $current_dhcp_dns1 = $1 ];then #如果相等就不用改了,
		...
		nvram set dhcp_dns2_x=$1
		nvram set dhcp_dns3_x=$1
		nvram commit
		restart_dhcpd
		/usr/bin/logger "[bypass detect] change dhcp_dns to $1 success"
		return 0
	fi
}
...
if [ $? -eq 0 ];then
	...
	echo "changeGateway $default_ip"
else
	changeGateway $bypass
	echo "changeGateway $bypass"
fi
